...
static char characterNameToChar(Token *token) {
  if (!memcmp(token->start, "alarm", min(5, token->length))) return (char)0x07;
  ...
  // We crash the program if this happens because if it does, it's a programmer
